...
void RenderController::zoomChanged(int value)
    ...
    float newZoomScale = std::pow(10.0, (float) value / 100.0);
...
// calls the current objects subdivide routine
void RenderController::callSubdivide()
{
  renderWindow->directedEdgeSurface->subdivide->StartSubdivision();

  // file saving
  std::ofstream geometryFile;
  std::string fileName = renderWindow->directedEdgeSurface->fileName;

  // split apart to just get object name
  int start = fileName.rfind('/');
  std::string shortName = fileName.substr(start + 1);
  shortName = shortName.substr (0, shortName.length() - 14);

  // find vertex count
  std::string vertCount = std::to_string((int)renderWindow->directedEdgeSurface->vertices.size());

  // construct string
  std::string finalName = "subdivisions/" + shortName + vertCount + ".diredgenormal";

  // save with WriteObjectStream function
  geometryFile.open(finalName);
  renderWindow->directedEdgeSurface->WriteObjectStream(geometryFile);
  geometryFile.close();
}
